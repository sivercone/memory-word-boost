...
interface UpsertUserOptions {
  users: ReadonlyArray<Types.UserModel>;
  data: Readonly<Partial<Types.UserModel> & Pick<Types.UserModel, 'email'>>;
  allowCreate?: boolean;
}
...
export function upsertUser({ users, data, allowCreate = true }: UpsertUserOptions): {
  ...
  if (index !== -1) {
    ...
    const updated = { ...existing, ...data, updatedAt: timestamp } satisfies Types.UserModel;
    return {
  ...
  return [
    {
      id: nanoid(),
      name: 'Famous Quotes',
      description: 'Learn quotes from famous people in history',
      folderId: 'sets',
      userId,
      createdAt: now,
      updatedAt: now,
